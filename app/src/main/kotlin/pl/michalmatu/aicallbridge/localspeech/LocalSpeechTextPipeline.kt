...
import pl.michalmatu.aicallbridge.textagent.TextCallFinalTurnDispatcher
...
/**
 * One conservative LOCAL_STT_TTS turn. It never emits PCM until the complete backend response or
 * exact deterministic candidate has passed the application-owned text approval policy.
 */
internal class LocalSpeechTextPipeline(
    ...
    private val dispatcher = TextCallFinalTurnDispatcher(controller)
    private val gate = LocalSpeechGenerationGate()

    fun start(
        listener: Listener,
        finalTurnRouteSelector: LocalSpeechFinalTurnRouteSelector? = null,
    ) {
        cancel()
        ...
        input.start(object : OnDeviceSpeechInput.Listener {
            ...
            override fun onFinalTranscript(text: String) {
                ...
                try {
                    val route = selectLocalSpeechFinalTurnRoute(text, finalTurnRouteSelector)
                    dispatcher.dispatch(text, route, object : TextCallFinalTurnDispatcher.Listener {
                    ...
                    finishError(generation, listener, "final_turn_route_${error.javaClass.simpleName}")
                }
            }
