...
class CallRealtimeInstructionsBuilder {
    fun build(task: CallTask): String {
        ...
        return buildString {
            ...
            appendLine("Do not state or imply a commitment before the full application-owned commitment sequence completes.")
            appendLine("A successful evaluate_proposal does not authorize a commitment by itself; it may return a commitment_authorization permit for that exact proposal.")
            appendLine("When evaluate_proposal returns commitment_authorization, call commit_proposal immediately with the exact authorization value and do not speak an acceptance or confirmation first.")
            appendLine("Only after commit_proposal reports that the commitment is authorized may you state or imply to the counterparty that the proposal is accepted, booked, purchased, confirmed, or otherwise committed.")
            appendLine("If evaluate_proposal reports that a user decision is required, do not commit and wait for the application to resolve that exact decision.")
            appendLine("If evaluate_proposal returns user_rejected, continue negotiating or asking for a different proposal; do not reuse or infer any prior authorization.")
