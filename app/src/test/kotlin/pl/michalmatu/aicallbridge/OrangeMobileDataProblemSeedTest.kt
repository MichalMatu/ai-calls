...
class OrangeMobileDataProblemSeedTest {
    @Test
    fun `orange mobile data problem seed uses one reviewed non committing utterance`() {
        val scenario = GateCLiveCallScenarioFactory.create(
            GateCLiveCallScenarioFactory.ORANGE_SUPPORT_NUMBER,
            OrangeLiveAction.MOBILE_DATA_PROBLEM,
        )
        val observedRoot =
        ...
        assertEquals("Nie działają mi dane komórkowe.", decision.text())
