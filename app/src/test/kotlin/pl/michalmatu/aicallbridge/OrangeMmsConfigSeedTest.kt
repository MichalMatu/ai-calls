...
class OrangeMmsConfigSeedTest {
    @Test
    fun `orange mms config seed uses one reviewed non committing utterance`() {
        val scenario = GateCLiveCallScenarioFactory.create(
            GateCLiveCallScenarioFactory.ORANGE_SUPPORT_NUMBER,
            OrangeLiveAction.MMS_CONFIG,
        )
        val observedRoot =
        ...
        assertEquals("Jak skonfigurować MMS w telefonie?", decision.text())
