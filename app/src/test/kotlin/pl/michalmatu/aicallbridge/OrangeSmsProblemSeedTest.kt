...
class OrangeSmsProblemSeedTest {
    @Test
    fun `orange sms problem seed uses one reviewed non committing utterance`() {
        val scenario = GateCLiveCallScenarioFactory.create(
            GateCLiveCallScenarioFactory.ORANGE_SUPPORT_NUMBER,
            OrangeLiveAction.SMS_PROBLEM,
        )
        val observedRoot =
        ...
        assertEquals("Nie mogę wysyłać SMS-ów.", decision.text())
