...
- `clir_iteration.sh --allow-clir-enable` — run the current guarded physical CLIR enable iteration through the existing `relay-live` runner, then require the phone to return to `IDLE`. It preserves the runner's explicit CLIR-effect guard and does not duplicate call/media/dialogue logic.
